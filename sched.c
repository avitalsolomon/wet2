...
#define ms_to_ticks(x) (HZ * (x) / 1000)
#define ticks_to_ms(x) (1000 * (x) / HZ)
...
int sys_short_remaining_time (pid_t pid) {
	...
	if (is_short(pid)) {
		//TODO return he time letf before it becomes overdue;
	}
	//TODO overdue - return the time left before it becomes SCHED OTHER
}
...
struct runqueue {
	...
	task_t *curr, *idle;
	prio_array_t *active, *expired, *short_proc, *overdue_short_proc, arrays[4]; /*HW2*/
...
asmlinkage long sys_sched_yield(void)
{
	...
	/* HW2 - if this->prio < MAX_RT_PRIO=MAX_USER_RT_PRIO */
	if ( unlikely( rt_task(current) || current->policy == SCHED_SHORT ) ) {
		list_del(&current->run_list);
		if (current->policy == SCHED_SHORT && current->is_overdue) {
			list_add_tail(&current->run_list, array->queue + STATIC_OVERDUE_PRIO);
		} else {
...
void __init sched_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		/* HW2 - start */
		rq->short_proc = rq->arrays + 2;
		rq->overdue_short_proc = rq->arrays + 3;
		/* HW2 - end */
